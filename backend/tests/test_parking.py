...
def test_start_session(client, make_user, auth_headers):
    ...
    resp = client.post("/v1/student/start", headers=headers,
...
def test_duplicate_active_session_rejected(client, make_user, auth_headers):
    ...
    client.post("/v1/student/start", headers=headers, json={"plate": "PKD001", "zone": "B2"})
    resp = client.post("/v1/student/start", headers=headers, json={"plate": "PKD002", "zone": "C3"})
...
def test_get_active_session(client, make_user, auth_headers):
    ...
    client.post("/v1/student/start", headers=headers, json={"plate": "PKA001", "zone": "D4"})
    resp = client.get("/v1/student/active", headers=headers)
    assert resp.status_code == 200
...
def test_end_session(client, make_user, auth_headers):
    ...
    client.post("/v1/student/start", headers=headers, json={"plate": "PKE001", "zone": "E5"})
    resp = client.post("/v1/student/end", headers=headers)
    assert resp.status_code == 200

    # Active session should now be gone
    resp2 = client.get("/v1/student/active", headers=headers)
...
def test_session_history(client, make_user, auth_headers):
    ...
    client.post("/v1/student/start", headers=headers, json={"plate": "PKH001", "zone": "F6"})
    client.post("/v1/student/end", headers=headers)

    resp = client.get("/v1/student/my-sessions", headers=headers)
    assert resp.status_code == 200
    sessions = resp.json()
...
def test_expired_session_not_returned_as_active(client, make_user, auth_headers, db):
    ...
    start = client.post("/v1/student/start", headers=headers,
