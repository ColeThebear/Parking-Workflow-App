...
def test_admin_list_all_citations(client, make_user, auth_headers):
    ...
    client.post("/enforcement/citations", headers=enf_headers, json={
        "vehicle_plate": "ADM001", "zone": "A1", "violation_type": "No Valid Session",
    })

    resp = client.get("/admin/citations", headers=admin_headers)
    assert resp.status_code == 200
...
def test_admin_mark_citation_paid(client, make_user, auth_headers):
    ...
    cite_resp = client.post("/enforcement/citations", headers=enf_headers, json={
        "vehicle_plate": "PAD001", "zone": "B2", "violation_type": "Expired Session",
    })
    citation_id = cite_resp.json()["id"]

    resp = client.patch(f"/admin/citations/{citation_id}/paid", headers=admin_headers)
    assert resp.status_code == 200
    assert resp.json()["success"] is True

    # Verify in the admin citation list
    list_resp = client.get("/admin/citations", headers=admin_headers)
...
def test_admin_mark_nonexistent_citation_paid(client, make_user, auth_headers):
    ...
    resp = client.patch("/admin/citations/999999/paid", headers=headers)
...
def test_admin_credit_balance(client, make_user, auth_headers, db):
    ...
    resp = client.post("/admin/balance/credit", headers=admin_headers, json={
        "user_id": student.id,
        "amount": 1000,
...
def test_admin_reset_password(client, make_user, auth_headers):
    ...
    resp = client.post("/admin/reset-password", headers=admin_headers, json={
        "user_id": target.id,
    })
    assert resp.status_code == 200
    data = resp.json()
...
def test_admin_reset_password_unknown_user(client, make_user, auth_headers):
    ...
    resp = client.post("/admin/reset-password", headers=headers, json={"user_id": 999999})
    assert resp.status_code == 404
